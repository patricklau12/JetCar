...
<body>
  <div class="wrap">
    <div class="hero">
      <h1>JetCar Contour + YOLO Panel</h1>
      <p>Keep the current contour-based line following on the lower ROI, and add YOLO object detection on the full RGB frame with boxes drawn on the live RGB view.</p>
      <div class="grid">
  ...
  <script>
    ...
    async function refreshStatus() {
      ...
      try {
        ...
        document.getElementById('yoloState').innerText = data.yolo ? data.yolo.status : 'unknown';
        if (data.settings) applySettings(data.settings);
        if (data.analysis) {
          const a = data.analysis;
          const yoloNames = (a.yolo_detections || []).slice(0, 8).map((d) => `${d.label}:${d.conf.toFixed(2)}`).join(', ');
...
class CameraAnalyzer:
    def __init__(
        ...
        yolo_model_name: str = YOLO_DEFAULT_MODEL,
        yolo_conf: float = YOLO_DEFAULT_CONF,
        yolo_imgsz: int = YOLO_DEFAULT_IMGSZ,
        ...
        self._yolo_every_n = max(1, int(yolo_every_n))
        self._yolo_device = yolo_device
        self._yolo_max_det = int(yolo_max_det)
    ...
    def _init_yolo(self) -> None:
        if UltralyticsYOLO is None:
            self._yolo_error = "ultralytics not installed"
            return
        try:
            self._yolo_model = UltralyticsYOLO(self._yolo_model_name)
            names = getattr(self._yolo_model, "names", {}) or {}
            if isinstance(names, list):
                self._yolo_names = {i: str(name) for i, name in enumerate(names)}
            elif isinstance(names, dict):
                self._yolo_names = {int(k): str(v) for k, v in names.items()}
            else:
                self._yolo_names = {}
    ...
    def yolo_status(self) -> dict[str, Any]:
        status = "ready" if self._yolo_model is not None else f"offline: {self._yolo_error}"
        return {
            "loaded": self._yolo_model is not None,
            "status": status,
            "model": self._yolo_model_name,
        ...
        try:
            ...
            if result is not None and getattr(result, "boxes", None) is not None and len(result.boxes) > 0:
                ...
                for box, conf, cls_id in zip(boxes_xyxy, confs, classes):
                    ...
                    detections.append({
                        "label": self._yolo_names.get(int(cls_id), str(int(cls_id))),
                        "conf": float(conf),
                        "cls": int(cls_id),
                        "xyxy": [x1, y1, x2, y2],
                    })
    ...
    @staticmethod
    def _draw_yolo_boxes(image: np.ndarray, detections: list[dict[str, Any]], status_text: str | None = None) -> np.ndarray:
        out = image.copy()
        for det in detections:
            ...
            y_text = max(18, y1 - 8)
            cv2.putText(out, label, (x1, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 2, cv2.LINE_AA)
        if status_text:
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                self._frame_index += 1
                if self._frame_index % self._yolo_every_n == 0:
...
class AppState:
    ...
    def status_payload(self) -> dict[str, Any]:
        return {
            ...
            "yolo": self.analyzer.yolo_status() if self.analyzer is not None else {"loaded": False, "status": "offline: no analyzer"},
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--yolo-model", default=YOLO_DEFAULT_MODEL)
    parser.add_argument("--yolo-conf", type=float, default=YOLO_DEFAULT_CONF)
    parser.add_argument("--yolo-imgsz", type=int, default=YOLO_DEFAULT_IMGSZ)
